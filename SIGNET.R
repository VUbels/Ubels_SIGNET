...
Genesets <- function(copaired){
  ...
  genesets = genesets[grepl(',',genesets$gene_list), , drop = FALSE]
  if (dim(genesets)[1] == 0){
...
Prune <- function(outlier,genesets,motifRankings,motifAnnot,species=c('Homo sapiens','Mus musculus','Drosophila melanogaster')){
  ...
  for (q in 1:length(genesets_list)) {
    ...
    motifEnrichmentTable_wGenes <- cisTarget(genelist, motifRankings, motifAnnot=motifAnnot, nesThreshold = 2, aucMaxRank = 0.1 * ncol(motifRankings), geneErnMaxRank = 7000)
...
AUCellMatrix <- function(data,genesets,genesets_list){
  ...
  for(i in 1:length(genesets_list)){
    ...
    cells_AUC <- AUCell_calcAUC(geneSets, cells_rankings, aucMaxRank=nrow(cells_rankings)*0.2)
    cells_assignment <- AUCell_exploreThresholds(cells_AUC, plotHist=TRUE, nCores=1, assign=TRUE, thrP = 0.03, smallestPopPercent = 0.2)
...
AUCellMatrix_plots <- function(data, genesets, genesets_list){
  data_auc <- as.data.frame(matrix(0, length(genesets_list), dim(data)[2]))
  cells_rankings <- AUCell_buildRankings(as.matrix(data))
  threshold = numeric()
  
  for(i in 1:length(genesets_list)){
    # Get current gene name for the plot title
    current_gene <- genesets_list[i]
    
    # Open PNG device for saving plot
    png(filename = paste0(current_gene, "_AUCell_Threshold.png"), 
        width = 800, height = 600, res = 100)
    
    genes <- unlist(strsplit(genesets[i,1], split = ","))
    geneSets <- list(geneSet1=genes)
    geneSets <- GSEABase::GeneSet(genes, setName=genesets_list[i])
    cells_AUC <- AUCell_calcAUC(geneSets, cells_rankings, aucMaxRank=nrow(cells_rankings)*0.3)
    cells_assignment <- AUCell_exploreThresholds(cells_AUC, plotHist=TRUE, nCores=1, 
                                                 assign=TRUE, thrP = 0.05, 
                                                 smallestPopPercent = 0.05)
